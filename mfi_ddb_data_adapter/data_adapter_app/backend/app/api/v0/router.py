...
async def lifespan(_app):
    """
    On startup, restore connections that were active before the last
    shutdown/crash from the SQLite store. Each restore is independent - one
    connection failing to reconnect (bad saved config, unreachable broker,
    etc.) is logged and skipped rather than blocking every other connection
    from coming back.
    """
    ...
    for entry in connection_store.load_all_connections(DB_PATH):
        ...
        try:
            ...
            if entry["desired_state"] in ("streaming", "paused"):
            ...
            logger.info("Restored connection conn_id=%s to desired_state=%s", conn_id, entry["desired_state"])
        except Exception as err:
            logger.warning("Failed to restore connection conn_id=%s, skipping: %s", conn_id, err)
