export const CONFIG_BLUEPRINT = {
  infra: {
    ...
    fields: [
      // 1. Target selector added to see if they want our container or their own instance
      { key: "INFRA_DEPLOYMENT", label: "Broker Deployment Target", type: "select", options: ["internal", "external"], default: "internal" },
      // 2. Swapped text to allow direct mapping of any external IP/Domain, defaulting to internal container name
      { key: "MQTT_BROKER_HOST", label: "Broker Hostname / IP Address", type: "text", default: "mqtt-broker" },
      { key: "MQTT_BROKER_PORT", label: "Broker Connection Port", type: "number", default: 1883 },
  ...
  kv: {
    ...
    fields: [
      // Target Selector
      { key: "KV_DEPLOYMENT", label: "KV Deployment Target", type: "select", options: ["internal", "external"], default: "internal" },
      // Host Field Added (defaults to its compose alias "kv-psql-db")
      { key: "KV_DB_HOST", label: "Database Hostname / IP Address", type: "text", default: "kv-psql-db" },
      { key: "KV_DB_HOST_PORT", label: "Database Target Port", type: "number", default: 5431 },
  ...
  ts: {
    ...
    fields: [
      // Target Selector
      { key: "TS_DEPLOYMENT", label: "Timescale Deployment Target", type: "select", options: ["internal", "external"], default: "internal" },
      // Host Field Added (defaults to its compose alias "timescaledb-db")
      { key: "TS_DB_HOST", label: "Database Hostname / IP Address", type: "text", default: "timescaledb-db" },
      { key: "TS_DB_HOST_PORT", label: "Database Target Port", type: "number", default: 5432 },
  ...
  rws: {
    ...
    fields: [
      // Target Selector
      { key: "RWS_DEPLOYMENT", label: "RWS / Metadata Deployment Target", type: "select", options: ["internal", "external"], default: "internal" },
      // Host Field Added (defaults to its compose alias "metadata-store-db")
      { key: "MDS_DB_HOST", label: "Metadata DB Hostname / IP Address", type: "text", default: "metadata-store-db" },
      { key: "RWS_API_PORT", label: "External Application REST Target Port", type: "number", default: 8000 },
      { key: "MDS_DB_HOST_PORT", label: "Metadata DB Connection Port", type: "number", default: 5430 },
