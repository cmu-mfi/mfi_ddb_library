...
class BlobTopicFamily(BaseTopicFamily):
    ...
    def process_data(self, data, is_attribute: bool = False):
        ...
        for key, value in data.items():
            self.__trial_id = value.get("trial_id", self.__trial_id)        
            payload.trial_id = self.__trial_id
            
            metric = blob_pb2.Payload.Metric()
            
            metric.name = "data" if not is_attribute else "attributes"
            metric.metadata.file_name = value.get("file_name", "")
            metric.metadata.file_type = value.get("file_type", "")
            metric.metadata.size = value.get("size", 0)
            metric.timestamp = value.get("timestamp", payload.timestamp)
            metric.bytes_value = value.get("file", b'')
            ...
            other_metadata['system_name'] = key
