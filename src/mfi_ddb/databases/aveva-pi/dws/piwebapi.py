...
class PIWebAPI:
    ...
    #     #Get WebId for all topics, if wildcard is used
    ...
    def __get_topic_webid(self, topic):
        ...
        # def __get_topic_webid(self, topic):
    #     """
    #     Always returns a list of WebIds.
    ...
    #         self.logger.debug(f"Using nameFilter: {name_filter}")
    ...
    #         )
    #         self.logger.debug(
    #             f"Constructed wildcard points request URL base: {get_request}"
    #         )

    #         parameters = {
    #             "nameFilter": name_filter,
    #             "selectedFields": "Items.WebId"
    #         }
    ...
    #         webids = [item["WebId"] for item in items if "WebId" in item]

    #         self.logger.info(
    #             f"Wildcard lookup complete for topic '{topic}'. "
    #             f"Found {len(webids)} matching WebIds."
    #         )

    #         if len(webids) > 0:
    #             self.logger.debug(f"Resolved WebIds: {webids}")
    ...
    #         self.logger.debug(f"Using PI path: {full_path}")
    ...
    #         if not webid:
    #             self.logger.warning(
    #                 f"No WebId found for exact topic lookup: {topic}"
    #             )
    #             return []

    #         self.logger.info(
    #             f"Resolved exact topic '{topic}' to WebId: {webid}"
    #         )
