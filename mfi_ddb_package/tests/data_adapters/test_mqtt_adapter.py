...
def test_system_polling():
    ...
    def publish_birth_data():
        ...
        while client.is_connected():
            client.publish(topic="devices/1/sensor-1/data", payload=payload)
            client.publish(topic="devices/2/sensor-2/data", payload=payload)
            time.sleep(1)
    ...
    client.publish(topic="devices/1/sensor-1/data", payload=payload_str)

    streamer.poll_and_stream_data()

    # PUBLISH TEST MESSAGE 2
    payload_str = json.dumps({"room-1": {"temp": 70}, "room-2": {"temp": 55.5}})
    client.publish(topic="devices/2/sensor-2/data", payload=payload_str)
    payload_str = json.dumps({"room-1": {"humidity": 40}, "room-2": {"humidity": 30}})
    client.publish(topic="devices/2/sensor-3/data", payload=payload_str)
