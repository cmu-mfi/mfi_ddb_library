...
def main(mqtt_cfg_file, cfs_cfg_file):
    ...
    mqtt_sub.create_message_callback(topic, lambda message: callback(cfs_config, message))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Subscribe to a topic and save files to Cloud File Store (CFS) based on config."
    )
    parser.add_argument(
        "mqtt_config_path", help="Path to the MQTT configuration file (e.g., mqtt.yaml)."
    )
    parser.add_argument(
        "cfs_config_path", help="Path to the CFS configuration file (e.g., cfs.yaml)."
    )
