...
class MqttSpb:
    def __init__(self, config: dict, topic_family: BaseTopicFamily) -> None:
        ...
        if "mqtt" not in self.cfg:
            ...
            mqtt_keys = ["enterprise", "site", "broker_address"]
    ...
    def connect(self, component_ids: list):
        ...
        mqtt_tls_enabled = mqtt_cfg.get("tls_enabled", False)
        debug = mqtt_cfg.get("debug", False)

        for component_id in component_ids:
            spb_component = MqttSpbEntityDevice(
                group_name, edge_node_name, component_id, debug_enabled=debug
            )

            _connected = False
            while not _connected:
                _connected = spb_component.connect(
                    mqtt_host, mqtt_port, mqtt_user, mqtt_pass, mqtt_tls_enabled
                )
                if not _connected:
            ...
            data_topic = "%s/%s/%s/%s/#" % (  # noqa: UP031
                spb_component._spb_namespace,
    ...
    def publish_birth(self, attributes, data):
        ...
        for component_id in self._components:
            ...
            if component_id not in attributes and component_id not in data:
                print(
                    f"{component_id} attributes or data not available during birth.",
            ...
            if not self.__check_attributes(component_attr):
                raise Exception(
                    f"{self._topic_family.topic_family_name} not compatible with MqttSpb"
                )
            for key in component_attr:
    ...
    def __check_data(self, data: dict):
        for key in data:
            if type(data[key]) not in [int, float, str, list, bool]:
                return False
            if type(data[key]) is list and len(data[key]) > MAX_ARRAY_SIZE:
                return False
            if type(data[key]) is str and len(data[key]) > 255:
