...
class _Mqtt:
    ...
    def connect(self):
        ...
        timeout = mqtt_cfg.get("timeout", 5.0)
        ...
        if not self.client.is_connected():
            raise ConfigError(
                f"Could not connect to MQTT broker {mqtt_host} after {timeout} seconds."
            )
...
class _SCHEMA:
    class _MQTT(BaseModel):
        ...
        broker_port: Optional[int] = Field(
            1883, description="Port of the MQTT broker (default: 1883)"
        )
        ...
        debug: Optional[bool] = Field(
            False, description="Enable debug mode for MQTT client (default: False)"
        )
    ...
    class SCHEMA(BaseModel):
        ...
        queue_size: int = Field(
            10,
            description="Maximum number of messages to buffer before processing. FIFO.(default:10)",
        )
        topics: List["_TOPIC"] = Field(
            ...,
            description=(
                "List of topics to subscribe to. Required keys: ['component_id', 'topic']."
                "Optional: ['trial_id']"
            ),
        )
...
class MqttDataAdapter(BaseDataAdapter, _Mqtt):
    ...
    CONFIG_HELP = {
        "mqtt": {
            ...
            "timeout": "(optional) Timeout in sec for connecting to the MQTT broker (default:5s)",
        },
        "trial_id": "Trial ID for the system. No spaces or special characters allowed.",
        "queue_size": "(optional) Max number of messages to buffer before processing. (default:10)",
        "topics": (
            "List of topics to subscribe to. Required keys: ['component_id', 'topic']."
            "Optional: ['trial_id']"
        ),
    ...
    CONFIG_EXAMPLE = {
        ...
        "queue_size": 10,
        "topics": [
            {"component_id": "robot-arm-1", "topic": "robot-arm/1/data", "trial_id": "trial_001"},
            {"component_id": "machine-a", "topic": "machine/a/data"},
        ],
    }
    ...
    def __get_component_from_topic(self, topic_name):
        ...
        raise ConfigError(
            f"ERROR in file {__file__} line {sys._getframe().f_lineno}.\n"
