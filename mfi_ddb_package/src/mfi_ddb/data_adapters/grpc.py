...
class _SCHEMA(BaseModel):
    class _GRPCComponent(BaseModel):
        ...
        attributes: dict = Field(
            ..., description="Additional attributes for the gRPC component (optional)"
        )
        trial_id: str = Field(
            ...,
            description="Trial ID for the device. No spaces/special characters allowed (optional)",
        )
        proto_rel_path: str = Field(
            ..., description="Relative path to the .proto file defining the gRPC service"
        )
...
class GrpcDataAdapter(BaseDataAdapter):
    ...
    CONFIG_HELP = {
        ...
        "trial_id": "Trial ID for the gRPC device. No spaces/special characters allowed. (string)",
        "components": (
            "List of gRPC components to monitor. Keys:\n"
            "(component_id, attributes (optional), proto_path, stub_class, request_class, request)"
        ),
    }
        ...
        while i < len(self.cfg["components"]):
            ...
            if not request_module:
                if not self.did_i_compile_once:
                    ...
                    continue
                raise ImportError(
                    f"Could not import compiled protobuf request module for {proto_name}"
                )
