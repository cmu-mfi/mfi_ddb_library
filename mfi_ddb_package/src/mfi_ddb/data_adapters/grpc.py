...
class _SCHEMA(BaseModel):
    class _GRPCComponent(BaseModel):
        ...
        attributes: dict = Field({}, description="Additional attributes for the gRPC component")
        trial_id: str = Field(..., description="Trial ID for the device. No spaces or special char")
        proto_rel_path: str = Field(..., description="Relative path to the .proto file")
        request_method: str = Field(
            ...,
            description="Name of the gRPC service method to call.Only `Read` method is supported.",
        )
        stub_class: str = Field(
            ..., description="Name of the gRPC stub class to use for communication"
        )
        request_class: str = Field(
            ..., description="Name of the request class to use for the gRPC service call."
        )
    ...
    class SCHEMA(BaseModel):
        ...
        trial_id: str = Field(
            ..., description="Trial ID for the gRPC device. No spaces/special characters allowed."
        )
...
class GrpcDataAdapter(BaseDataAdapter):
    ...
    CONFIG_HELP = {
        ...
        "components": "List of gRPC components to monitor (list of dicts with keys: component_id,"
        "attributes (optional), proto_path, stub_class, request_class, request)",
    }
        ...
        "components": [
            {
                ...
                "attributes": {"description": "something something", "unit": "Celsius"},
                "proto_rel_path": "relative/path/to/temperature.proto",
                "stub_class": "TemperatureServiceStub",
                "request_class": "TemperatureRequest",
                "request": {"key1": "value1", "key2": [1, 2, 3]},
            },
            {
                "component_id": "sensor.humidity",
                "attributes": {"description": "something something", "unit": "Percentage"},
    ...
    def __init__(self, config: dict):
        ...
        try:
            ...
            raise ConnectionError("Error connecting to gRPC server") from None
        ...
        while i < len(self.cfg["components"]):
            ...
            if not request_module:
                if not self.did_i_compile_once:
                    ...
                    continue
                raise ImportError(
                    "Could not import compiled protobuf request module for", proto_name
                )
    ...
    def __generate_compiled_protos(self):
        ...
        result = subprocess.run(
            ["find", protopath, "-type", "f", "-print0"], capture_output=True, check=True
        )
        ...
        if isinstance(data_item, dict):
            ...
            for key in data_item:
                ...
                extracted_data.update(
                    self.__extract_key_value(data_item[key], f"{data_item_key}/{substitute_key}")
                )
