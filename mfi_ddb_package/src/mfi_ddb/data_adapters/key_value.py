...
class _SCHEMA(BaseModel):
    class _Payload(BaseModel):
        ...
        schema_version: str = Field(
            "mfi-v1.0", description="Version of kv schema to validate the payload."
        )
        msg_type: Literal["data", "project", "tp-tag", "user"] = Field(
            "data", description="OneOf('data', 'project', 'tp-tag', 'user')"
        )

    class SCHEMA(BaseModel):
        trial_id: str = Field(
            "mfi-ddb-metadata",
            description="(optional) It is required if one of the payload is of type 'data'",
        )
        payloads: List["_Payload"] = Field(
            ...,
            description="List of payload messages."
            "Payloads of type 'data', 'project', 'tp-tag', and 'user' allowed."
...
class KvDataAdapter(BaseDataAdapter):
    ...
    CONFIG_HELP = {
        ...
        "payloads": "List of payload messages."
        "Payloads of type 'data', 'project', 'tp-tag', and 'user' allowed."
        "Each will be verified against the kv.json schema.",
    }
    ...
    def __init__(self, config: dict):
        ...
        for payload in self.cfg["payloads"]:
            ...
            if payload["msg_type"] == "data" and self.cfg["trial_id"] == "mfi-ddb-metadata":
                self.logger.warning(
                    "WARNING: No trial_id provided. Payload of type 'data' is ignored."
                )
                continue
